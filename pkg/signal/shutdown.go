...
// @time      : 2024/7/29 下午5:24
// -------------------------------------------
package signal

import (
	"github.com/ZUCCzwp/kitex_ddd_example/pkg/onelog"
	"github.com/ZUCCzwp/kitex_ddd_example/pkg/threading"
	"os"
	"os/signal"
	"sync"
	"syscall"
...
func gracefulStop(signals chan os.Signal, sig syscall.Signal) {
	...
	onelog.Infof("Got signal %d, shutting down...", sig)
	...
	onelog.Infof("Still alive after %v, going to force kill the process...", delayTimeBeforeForceQuit)
